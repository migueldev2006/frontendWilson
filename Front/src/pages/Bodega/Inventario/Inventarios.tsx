...
export const InventariosTable = () => {
  const { inventarios, isLoading, isError, error, addInventario, changeState } =
    useInventario();
      ...
      render: (inventario: Inventario) => (
        <Chip
          className={`px-2 py-1 rounded ${
            inventario.estado ? "text-green-500" : " text-red-500" //color texto
          }`}
  ...
  const InventariosWithKey = inventarios
    ?.filter((inventario) => inventario?.id_inventario !== undefined)
    .map((inventario) => ({
      ...inventario,
